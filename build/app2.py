...
def sign_up(email, password, username):
    ...
    try:
        ...
        save_user_data(user.uid, user_data)
...
def save_weight_entry(weight, date):
    ...
    try:
        ...
        update_user_data(st.session_state.user_id, user_data)
        return True, "Weight entry saved successfully!"
    except Exception as e:
        return False, str(e)
...
def get_weight_history():
    ...
    try:
        user_data = get_user_data(st.session_state.user_id)
        if not user_data or 'weight_history' not in user_data:
            return None
        
        # Convert to DataFrame
        df = pd.DataFrame(user_data['weight_history'])
        df['date'] = pd.to_datetime(df['date'])
        return df.sort_values('date')
    except Exception as e:
        st.error(f"Error getting weight history: {str(e)}")
        return None

def update_goal_weight(goal_weight, goal_date):
    """Update goal weight in Firebase"""
    if not st.session_state.user_id:
        return False, "User not logged in!"
    ...
    try:
        ...
        user_data['goal_weight'] = goal_weight
        user_data['goal_date'] = goal_date.isoformat()
        
        update_user_data(st.session_state.user_id, user_data)
        return True, "Goal weight updated successfully!"
    except Exception as e:
        return False, str(e)

def delete_weight_entry(date):
    """Delete weight entry from Firebase"""
    if not st.session_state.user_id:
        return False, "User not logged in!"
    ...
    try:
        user_data = get_user_data(st.session_state.user_id)
        if not user_data or 'weight_history' not in user_data:
            return False, "No weight history found!"
        
        # Filter out the entry to delete
        user_data['weight_history'] = [
            entry for entry in user_data['weight_history']
            if entry['date'] != date.isoformat()
        ]
        
        update_user_data(st.session_state.user_id, user_data)
        return True, "Weight entry deleted successfully!"
    except Exception as e:
        return False, str(e)
...
def get_leaderboard():
    ...
    try:
        ...
        for user in users:
            if 'weight_history' in user and len(user['weight_history']) >= 2:
                # Calculate weight loss over the past week
                ...
                if len(recent_entries) >= 2:
                    weight_loss = recent_entries[0]['weight'] - recent_entries[-1]['weight']
        ...
        return sorted(leaderboard_data, key=lambda x: x['weight_loss'], reverse=True)
    except Exception as e:
