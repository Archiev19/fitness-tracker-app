...
st.markdown("""
    ...
    /* Enhanced section containers with hover effect */
    .section-container {
        background-color: var(--secondary-bg);
        padding: 1.2rem;
        border-radius: 12px;
    ...
    with col_main:
        ...
        st.markdown("""
            <div class="login-container">
                ...
                <div class="subtitle">
                    <p style="font-size: 1.2rem;">Your personal fitness journal! 💪</p>
                </div>
            </div>
        """, unsafe_allow_html=True)
...
def calorie_calculator_page(username):
    ...
    if "user_target_weight" not in st.session_state:
        st.session_state.user_target_weight = 70.0
    ...
    with st.expander("Calculate Your Fat Loss Calories", expanded=True):
        st.markdown("""
            <div style="text-align: center; margin-bottom: 1rem;">
        ...
        # Ensure fat_loss_rate is initialized
        if "user_fat_loss_rate" not in st.session_state:
            st.session_state.user_fat_loss_rate = "Moderate (0.5 kg/week)"
            
        fat_loss_options = ["Slow (0.25 kg/week)", "Moderate (0.5 kg/week)", "Aggressive (1 kg/week)"]
        fat_loss_rate = st.select_slider(
            "Rate of Fat Loss",
            options=fat_loss_options,
...
def main_page():
    ...
    if st.button("Add Weight Entry"):
        if db.add_weight_entry(username, date, weight):
    ...
    weight_history = db.get_weight_history(username)
    if not weight_history.empty:
        fig = go.Figure()
        fig.add_trace(go.Scatter(x=weight_history['date'], y=weight_history['weight'],
                               mode='lines+markers', name='Weight'))
        st.plotly_chart(fig)
        
        # Display weight table
        st.dataframe(weight_history)
    else:
        st.info("No weight entries yet. Start tracking your weight!")
        ...
        with col2:
            ...
            if backup_files:
                ...
                if st.button("Restore Selected Backup"):
                    if st.warning("This will overwrite current data. Are you sure?"):
                        backup_path = os.path.join(db.backup_dir, selected_backup)
            ...
            try:
                with open(db.db_file, 'rb') as f:
                    st.download_button(
                        ...
                        data=f,
                        file_name="fitness_tracker_backup.db",
